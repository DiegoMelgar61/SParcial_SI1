...
export default function Edit({ gestion, estados, semestres }) {
    const { data, setData, put, processing, errors } = useForm({
        codigo: gestion.codigo || '',
        nombre: gestion.nombre || '',
        anio: gestion.anio || new Date().getFullYear(),
        semestre: gestion.semestre || '',
        fecha_inicio: gestion.fecha_inicio || '',
        fecha_fin: gestion.fecha_fin || '',
        fecha_inicio_inscripciones: gestion.fecha_inicio_inscripciones || '',
        fecha_fin_inscripciones: gestion.fecha_fin_inscripciones || '',
        fecha_inicio_clases: gestion.fecha_inicio_clases || '',
        fecha_fin_clases: gestion.fecha_fin_clases || '',
        estado: gestion.estado || 'planificacion',
        observaciones: gestion.observaciones || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(`/gestiones-academicas/${gestion.id}`);
    };

    return (
        <AuthenticatedLayout>
            <Head title={`Editar ${gestion.nombre}`} />
                    ...
                    <div className="bg-white rounded-lg shadow-md p-6">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="flex items-center justify-end space-x-3 mt-6">
                                ...
                                >
                                    {processing ? 'Actualizando...' : 'Actualizar Gestión'}
                                </button>
                            </div>
                        </form>
                    </div>
