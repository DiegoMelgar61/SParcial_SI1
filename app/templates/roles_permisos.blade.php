        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody id="tabla-roles" class="bg-white divide-y divide-gray-200 text-sm">
            ...
            <tr class="hover:bg-gray-50 transition">
              ...
              <td class="px-6 py-4 font-medium text-gray-800">{{ $rol['nombre'] }}</td>
              <td class="px-6 py-4 text-gray-600">{{ $rol['descripcion'] ?? '—' }}</td>
              <td class="px-6 py-4">
                @if(!empty($rol['permisos']))
                  @foreach($rol['permisos'] as $perm)
                    <span class="inline-block bg-indigo-50 text-indigo-700 text-xs px-2 py-1 rounded-md mr-1 mb-1">{{ $perm['nombre'] }}</span>
                  @endforeach
                @else
              ...
              <td class="px-6 py-4 text-center">
                <div class="flex justify-center gap-3">
                  <button data-id="{{ $rol['id'] }}" class="btn-edit text-indigo-600 hover:text-indigo-800 p-1 rounded-md hover:bg-indigo-50 transition" title="Editar Rol">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11"/>
                    </svg>
                  </button>
                  <button data-id="{{ $rol['id'] }}" class="btn-permisos text-blue-600 hover:text-blue-800 p-1 rounded-md hover:bg-blue-50 transition" title="Gestionar Permisos">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12H9m12 0a9 9 0 11-9-9 9 9 0 019 9z"/>
                    </svg>
                  </button>
                  <button data-id="{{ $rol['id'] }}" class="btn-delete text-red-600 hover:text-red-800 p-1 rounded-md hover:bg-red-50 transition" title="Eliminar Rol">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
